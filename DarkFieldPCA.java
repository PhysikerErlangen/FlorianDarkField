...
public class DarkFieldPCA {
	...
	public static void main(String[] args) {
		...
		double[][] multi = new double[][] { { 1, 1, 0 }, { 1, 1, 0 },
				{ 0, 2, 3 }, { 1, 1, 0 }, { 1, 1, 1 }, { 5, 4, 4 },
				{ 5, 4, 4 }, };
		...
		DarkFieldPCA myPCA = new DarkFieldPCA(scatterDirections,
				myScatterWeights);
	...
	/**
	 * Data-Set consists of a set of many vectors of the same dimensions. The
	 * number of points is saved in numPoints
	 */
	private int numPoints;

	/**
	 * Data-Set consists of a set of many vectors of the same dimensions. This
	 * dimension is saved in dimension
	 */
	...
	/**
	 * covMatrix contains the entries covariance matrix of the given the data
	 * set
	 */
	...
	/**
	 * Constructor for DarkFieldPCA. CAUTION: DarkFieldPCA needs special data,
	 * as no mean shift is performed, as the data points are already assumed to
	 * have a mean of 0.
	 * 
	 * @param scatterDirections
	 * @param myScatterCoef
	 */
	public DarkFieldPCA(SimpleMatrix scatterDirections,
			SimpleVector myScatterCoef) {

		assert (scatterDirections.getCols() != myScatterCoef.getLen()) : new Exception(
				"Dimensions do not match in DarkFieldPCA.");
		...
		// 2*numSamples because we take + and - direction of the scatter
		// direction
		...
		// First calculate the "Dataset" that is used for PCA. This dataset has
		// already by definition mean 0
		calcSetOfScatterPoints();

	}

	/**
	 * Calculates set of 2K directions vectors corresponding to scatter
	 * coefficients and the respective scatter directions See eq. 15/16 of Vogel
	 * "Constrained X-Ray tensor tomography reconstruction" 2015
	 */
	 ...
	 * "Constrained X-Ray tensor tomography reconstruction" 2015
	 */
	public static SimpleMatrix calcSetOfScatterPoints(
			SimpleMatrix scatterDirections, SimpleVector scatterCoef) {

		// Check for inconsistency (different dimensions)
		assert (scatterDirections.getCols() == scatterCoef.getLen() && scatterDirections
				.getRows() == 3) : new Exception("Dimension of data is wrong.");
		...
		for (int direction = 0; direction < scatterCoef.getLen(); direction++) {
			...
			// Scattering coefficients are extracted after reconstruction of all
			// n_k
			// by component-wise application of the square root:
			double myWeight = Math.sqrt(Math.abs(scatterCoef
	...
	private void calcCovarianceMatrix() {
		...
		assert (scatterPoints != null) : new Exception(
				"Initialize data array fist.");
		...
		for (int ind = 0; ind < numPoints; ind++) {
			...
			SimpleMatrix outerMatrix = SimpleOperators.multiplyOuterProd(
					curVec, curVec);
	...
	/**
	 * Plots the data in the array over its array index. Written my Matthias
	 * Unberath. Adapted by Florian Schiffers
	 * 
	 * @param data
	...
	private void plot(double[] data) {
		...
		new ImageJ();
		Plot plot = VisualizationUtil
				.createPlot(data, "Singular values of data matrix",
	...
	public void showDataPoints() {
		...
		DarkFieldPointCloudViewer myViewer = new DarkFieldPointCloudViewer(
				scatterPoints);
